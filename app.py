...
if df_sheet.empty:
    ...
    def susun_nama_dropdown(row):
        ...
        s_name = row.get('site_name') if pd.notna(row.get('site_name')) else "Belum Terdata di Supabase"
    ...
    if pd.notna(data_site.get('parsed_timestamp')):
        formatted_ts = data_site['parsed_timestamp'].strftime('%d/%m/%Y %H:%M:%S')
    else:
        formatted_ts = raw_timestamp

    st.markdown(f"<p style='text-align: right; margin: -10px 5px 8px 0; font-size: 13px;'><b>Last Data Timestamp:</b> {formatted_ts}</p>", unsafe_allow_html=True)
                ...
                if not chart_data.empty:
                    ...
                    base = alt.Chart(df_altair).encode(
                        x=alt.X(f'{col_date}:T', 
                                ...
                                axis=alt.Axis(format='%d/%m/%Y', labelOverlap=True, title=None))
                    )
                    line_avail = base.mark_line(color='#00E5FF', strokeWidth=2, interpolate='monotone').encode(
                        ...
                        tooltip=[
                            alt.Tooltip(f'{col_date}:T', title='Tanggal', format='%d/%m/%Y'),
    ...
    # --- ROW 3: EVIDENCE & DOCUMENTATION SLIDE ---
